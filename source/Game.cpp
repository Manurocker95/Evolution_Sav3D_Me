...
Game::Game()
{
	...
	m_bgmInvencible = new sound(SND_BGM_INVINCIBLE, 1, true);
	m_sfxBanana = new sound(SND_SFX_BANANA, 3, false, false);
	m_sfxFire = new sound(SND_SFX_FIRE, 3, false, false);
...
void Game::ShowMessage(std::string msg, int x, int y, int size, u32 color, gfxScreen_t screen, gfx3dSide_t side, bool sftdlib)
{
	if (sftdlib)
		sftd_draw_text(font, x, y, color, size, msg.c_str());
	else
